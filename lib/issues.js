...
module.exports = {
  ...
  format: function (issues) {
    ...
    issues = issues
      .map(issue => {
        statuses[issue.Status] = statuses[issue.Status] ? statuses[issue.Status] + 1 : 1
        return {
          title: `${issue.Key}`,
    ...
    log(statuses)
    const statusCount = Object.keys(statuses).reduce((acc, status) => {
      acc.push(`${status} - ${statuses[status]}`)
      return acc;
    }, []).join(' | ')
    issues.unshift({
      title: `Total Issues - ${totalIssues}`,
      subtitle: statusCount,
      valid: false,
    })
    return issues;
  },
